# day10test
inputFile <- file('data/day10/input')
...
for(i in 1:length(x)){
  ...
  if(corrupted){
    ...
    # closing string
    rem.brackets <- mystack$stack()
    s <- 0
    sapply(rem.brackets, function(ob){
      s <<- 5*s+grep(ob, o.brakets, fixed = TRUE)
    })

    incomplete <- rbind(incomplete, data.frame(idx=j, line=x[i], score=s))
  }
}
